...
void CGameFramework::ProcessInput()
{
	...
	DWORD dwDirection = 0;
	/* 키보드와 상태 정보를 반환한다. 화살표 키(방향키)를 누르면 플레이어를 오른쪽/왼쪽(로컬 x-축), 앞/뒤(로컬 z-축)로
	이동한다. 'Page Up'과 'Page Down' 키를 누르면 플레이어를 위/아래(로컬 y-축)로 이동한다. */
	...
	메시지(WM_LBUTTONDOWN, WM_RBUTTONDOWN)를 처리할 때 마우스를 캡쳐하였다. 그러므로 마우스가 캡쳐된 것은
	마우스 버튼이 눌려진 상태를 의미한다. 마우스 버튼이 눌러진 상태에서 마우스를 좌우 또는 상하로 움직이면 플레이어를 x-축 또는 y-축으로 회전한다. */
	...
	if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
	{
		if (cxDelta || cyDelta)
		{
			/* ccxDelta는 y-축의 회전을 나타개 cyDelta는 x-축의 회전을 나타낸다. 오른쪽 마우스 버튼이 눌려진 경우 cxDelta는 z-축의 회전을 나타낸다. */
...
void CGameFramework::FrameAdvance() {
	...
	// 렌더 타겟은 그대로 두고 깊이 버퍼를 1.0ㅇ로 지우고 플레이어를 렌더링하면 플레이어는 무조건 그려질 것이다.
