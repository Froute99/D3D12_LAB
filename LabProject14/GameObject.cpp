	...
	XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color) : CGameObject(0)
{
	...
	/* 지형 객체는 격자 메쉬들의 배열로 만들 것이다. nBlockWidth, nBlockLength는
	격자 메쉬 하나의 가로, 세로 크기이다. cxQuadsPerBlock, czQuadsPerBlock은
	...
	pShader->CreateShader(pd3dDevice, pd3dGraphicsRootSignautre);
	SetShader(pShader);
}
